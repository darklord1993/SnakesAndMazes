Shader "Custom/TileShader" 
{
	...
	SubShader 
	{
		...
		float perm(float x, float y)
		{
			...
			float2 t = float2(floor(d)/(noiseResolution-1.0f+0.1f));
		...
		float4 frag(v2f i) : COLOR
		{
			...
			float n = noise2d(float2(i.texCoord.xy)*(noiseResolution - 1.0f)/noiseResolution);
			n = 0.5f*n + 0.5f;
			
			float4 outColor = float4(n,n,n,1.0f);
			
			//float4 outColor = tex2D(permSampler, i.texCoord);
			return outColor;
		}
		ENDCG
		} 
